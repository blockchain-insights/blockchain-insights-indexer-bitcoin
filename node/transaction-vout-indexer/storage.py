...
class Storage:
    ...
    def get_output(self, txid_vout_pairs):
        ...
        Returns: list of (address, value) tuples in same order as input
        """
        txid_vouts = [f"{txid}-{vout}" for txid, vout in txid_vout_pairs]
        ...
        output_map = {row[0]: (row[1], row[2]) for row in result.result_rows}
        
        # Return results in same order as input, None for missing entries
        return [output_map.get(tv) for tv in txid_vouts]
